...
with tf.name_scope('error-multitask'):
    error_multitask = tf.reduce_mean(error + 0.5*error_coarse)
...
step = 0
min_valid_error = (None, 10000000)
max_valid_accuracy = (None, 0)
training = train_step_multitask
switched = False
for e in range(num_epoch):
    ...
    elif e > min_valid_error[0] + 15 and not switched:
        training = train_step
        print('\n--------- SWITCHING TO FINAL TUNING ------\n')
        switched = True

    if max_valid_accuracy[1] < valid_accuracy[e]:
        max_valid_accuracy = (e, valid_accuracy[e])
    elif e > max_valid_accuracy[0] + 15:
        break
